...
async function fetchedData() {
  ...
  try {
    ...
    // the default when page load for first time or reload
  ...
  card.forEach(() => {
    if (history == 'Yesterday') {
      for (let i = 0; i < data.length; i++) {
        jsonData[i].textContent = `${data[i].timeframes.daily.current} ${data[i].timeframes.daily.current > 1 ? "hrs" : "hr"
          }`;
        jsonPre[i].textContent = `${history} - ${data[i].timeframes.daily.previous
